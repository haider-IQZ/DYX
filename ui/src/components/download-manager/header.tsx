...
interface HeaderProps {
  onAddDownload: (url: string, filename?: string) => Promise<void> | void;
...
function formatSpeed(bytesPerSecond: number) {
  ...
  return `${(bytesPerSecond / Math.pow(1024, index)).toFixed(1)} ${units[index]}`;
}
...
export function Header({
  ...
  const handleAddDownload = async () => {
    if (!url.trim()) return;
    await onAddDownload(url.trim(), filename.trim() || undefined);
    setUrl('');
    setFilename('');
    setDialogOpen(false);
  };
          ...
          <div className="flex items-center gap-1">
            <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
              <DialogTrigger asChild>
                <Button size="sm" className="gap-2 rounded-lg px-4" disabled={!axelAvailable}>
              ...
              <DialogContent className="sm:max-w-lg">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Enter a URL to download. Axel will use multiple connections for faster speeds.
                  </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2">
                    ...
                    <Input
                      ...
                      onKeyDown={(event) => event.key === 'Enter' && void handleAddDownload()}
                ...
                <DialogFooter>
                  ...
                  <Button onClick={() => void handleAddDownload()} disabled={!url.trim() || !axelAvailable}>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => void onPauseAll()} className="h-8 w-8">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => void onResumeAll()} className="h-8 w-8">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <Button variant="ghost" size="icon" onClick={() => void onClearCompleted()} className="h-8 w-8">
        ...
        <div className="grid grid-cols-2 gap-px border-t border-border bg-border md:grid-cols-4">
          <div className="flex items-center gap-3 bg-card/80 px-5 py-3">
            ...
            <div>
              ...
              <p className="font-mono text-lg font-semibold tabular-nums text-foreground">{activeCount}</p>
            </div>
          </div>
          ...
          <div className="hidden items-center gap-3 bg-card/80 px-5 py-3 md:flex">
            ...
            <div>
              ...
              <p className="font-mono text-lg font-semibold tabular-nums text-foreground">{totalDownloads}</p>
            </div>
          </div>
